...
handle_extension() {
    case "${FILE_EXTENSION_LOWER}" in
        ...
        htm|html|xhtml)
            # Preview as text conversion
            wkhtmltopdf "${FILE_PATH}" "${IMAGE_CACHE_PATH%.*}.pdf" && exit 6
...
handle_image() {
    ...
    case "${mimetype}" in
        ...
        # Markdown
        text/markdown)
            pandoc --from markdown+fancy_lists+startnum+task_lists --pdf-engine=lualatex -s -o "${IMAGE_CACHE_PATH%.*}.html" "${FILE_PATH}" || exit 2
            FILE_PATH="${IMAGE_CACHE_PATH%.*}.html"
            ;&
        # HTML
        text/html)
            wkhtmltopdf -B 0 -R 0 -L 0 -T 0                             \
                        --user-style-sheet ~/.config/ranger/custom.css  \
                        --background                                    \
                        "${FILE_PATH}"                                  \
                        "${IMAGE_CACHE_PATH%.*}.pdf" || exit 2
            FILE_PATH="${IMAGE_CACHE_PATH%.*}.pdf"
            ;&
        # PDF
        application/pdf)
            pdftoppm -f 1 -l 1 \
...
handle_fallback() {
    echo '----- File Type Classification -----' && xdg-mime query filetype "${FILE_PATH}" && exit 5
    exit 1
}
